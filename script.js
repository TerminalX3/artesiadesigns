...
const productsDB = {
  ...
  timelesstraditions: {
    ...
    size: '96" x 48"',
    price: "8999 each; Rent: $999 each/day",
    inStock: true
  },
  gather_glow: {
    id: 'gather_glow',
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (heroVideo) {
    ...
    if (heroVideo.tagName === 'VIDEO') {
      heroVideo.playsInline = true; // Fix Safari autoplay
      heroVideo.setAttribute('webkit-playsinline', 'true'); // iOS Safari
      heroVideo.setAttribute('playsinline', 'true'); // Standard
      heroVideo.setAttribute('x5-playsinline', 'true'); // WeChat browser
      heroVideo.setAttribute('x5-video-player-type', 'h5'); // WeChat browser
    }
  }
  ...
  // Enhanced video loading for cross-browser compatibility
  function enhanceVideoCompatibility() {
    const videos = document.querySelectorAll('video');
    videos.forEach(video => {
      // Add cross-browser attributes
      ...
      if (video.src && !video.src.includes('github.io')) {
        // If local video fails, try GitHub Pages fallback
        video.addEventListener('error', function() {
          const originalSrc = video.src;
          const githubSrc = originalSrc.replace('videos/', 'https://sumedhkota.github.io/my-videos/');
          video.src = githubSrc;
          console.log('Switched to GitHub Pages fallback:', githubSrc);
        });
      }
    });
  }
  
  // Apply video compatibility enhancements
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  document.addEventListener('click', function(e) {
    ...
    console.log('Clicked product card:', productCard);
